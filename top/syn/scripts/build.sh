...
# add a default_tool to the `syn` target in top/plover.core and let
...
cat <<'EOF'
top/syn/scripts/build.sh — synthesis flow is not yet implemented.
...
To wire up a real flow:
  ...
  2. Add `default_tool: <vendor>` and the matching `tools:` block to the
     `syn` target in top/plover.core. Add the constraints file
     (top/syn/constraints/plover.sdc) to a `constraints` fileset.
  3. Run:
